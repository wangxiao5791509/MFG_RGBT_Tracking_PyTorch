...
def train_mdnet():
    ...
    ## set image directory
    if pretrain_opts['set_type'] == 'GTOT50': 
        img_home = '/DATA/wangxiao/Multi_Modal_RGBT_dataset_CSR/'
        data_path = '/DATA/wangxiao/dfg-rgbt-RTMDNet-master-train-on-50/50.pkl'
    ...
    for k, (seqname, seq) in enumerate(data.items()):
        ...
        videoPath_v = seq['v_videoPath']
        videoPath_i = seq['i_videoPath']
        # seqName = seq['seqName']
        gt = seq['gt_i'] 
        ...
        dataset[k] = RegionDataset(img_dir, img_list_v, img_list_i, videoPath_v, videoPath_i, gt, model.receptive_field, pretrain_opts)
    ...
    for i in range(pretrain_opts['n_cycles']):
        ...
        for j, k in enumerate(k_list):
            ...
            cropped_scenes_v, cropped_scenes_i, pos_rois, neg_rois= dataset[k].next()


            for sidx in range(0, len(cropped_scenes_v)):
                cur_scene_v = cropped_scenes_v[sidx]
                cur_scene_i = cropped_scenes_i[sidx]
                cur_pos_rois = pos_rois[sidx]
                cur_neg_rois = neg_rois[sidx]

                cur_scene_v = Variable(cur_scene_v)
                cur_scene_i = Variable(cur_scene_i)
                cur_pos_rois = Variable(cur_pos_rois)
                cur_neg_rois = Variable(cur_neg_rois)
                if pretrain_opts['use_gpu']:
                    cur_scene_v = cur_scene_v.cuda()
                    cur_scene_i = cur_scene_i.cuda()
                    cur_pos_rois = cur_pos_rois.cuda()
                    cur_neg_rois = cur_neg_rois.cuda()
                
                # pdb.set_trace() 
                cur_feat_map_v, cur_feat_map_i, augmented_feats = model(cur_scene_v, cur_scene_i, k, out_layer='conv3')
               	
                cur_pos_feats = model.roi_align_model(augmented_feats, cur_pos_rois)
                cur_pos_feats = cur_pos_feats.view(cur_pos_feats.size(0), -1)
                cur_neg_feats = model.roi_align_model(augmented_feats, cur_neg_rois)
        ...
        if cur_score > best_score:
            ...
            # torch.save(states, '/home/wangxiao/Downloads/ACM-MM-GML_RGBT_tracking/rgbt-RTMDNet-master/models/rgbt_rtmdnet.pth') 
            if pretrain_opts['use_gpu']:
                model = model.cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-set_type", default = 'GTOT50' )
    parser.add_argument("-padding_ratio", default = 5., type =float)
    parser.add_argument("-model_path", default ="./models/CBAM_dfg_rtmdnet_trained_on_50.pth", help = "model path")
