...
# sys.path.insert(0,'./naive_rgbt_TANet_module/')
...
# generator_path = './naive_rgbt_TANet_module/naive_TANet_rgbt_model.pkl'

# Generator = naive_Generator() 
...
def train(model, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer='fc4'):
    ...
    for iter in range(maxiter):
        ...
        if batch_neg_cand > batch_neg:
            ...
            for start in range(0,batch_neg_cand,batch_test):
                ...
                if batch_neg_feats[start:end].shape[1] == 9216: 
                    ...
                    temp_neg_feats = torch.cat((batch_neg_feats[start:end], batch_neg_feats[start:end]), dim=1)  

                score = model(temp_neg_feats, temp_neg_feats, in_layer=in_layer)
                if start==0:
        ...
        if batch_pos_feats.shape[1] == 9216: 
            ...
            temp_pos_feats = torch.cat((batch_pos_feats, batch_pos_feats), dim=1)  

        if batch_neg_feats.shape[1] == 9216: 
            temp_neg_feats = batch_neg_feats 
        else: 
            temp_neg_feats = torch.cat((batch_neg_feats, batch_neg_feats), dim=1)  
...
def run_mdnet(img_list_v, img_list_i, init_bbox, gt=None, seq='seq_name ex)Basketball', savefig_dir='', display=False):
    ...
    init_targetObject_v = cur_image_v[int(init_bbox[0]):int(init_bbox[0]+init_bbox[2]), int(init_bbox[1]):int(init_bbox[1]+init_bbox[3]), :]   
    init_targetObject_i = cur_image_i[int(init_bbox[0]):int(init_bbox[0]+init_bbox[2]), int(init_bbox[1]):int(init_bbox[1]+init_bbox[3]), :] 


    # Draw pos/neg samples
    ishape = cur_image_v.shape
    pos_examples = gen_samples(SampleGenerator('gaussian', (ishape[1],ishape[0]), 0.1, 1.2), target_bbox, opts['n_pos_init'], opts['overlap_pos_init'])
    neg_examples = gen_samples(SampleGenerator('uniform', (ishape[1],ishape[0]), 1, 2, 1.1), target_bbox, opts['n_neg_init'], opts['overlap_neg_init'])
    neg_examples = np.random.permutation(neg_examples)

    cur_bbreg_examples = gen_samples(SampleGenerator('uniform', (ishape[1],ishape[0]), 0.3, 1.5, 1.1), target_bbox, opts['n_bbreg'], opts['overlap_bbreg'], opts['scale_bbreg'])
    ...
    for bidx in range(0,scene_boxes.shape[0]):
        ...
        cropped_image_v, cur_image_var_v = img_crop_model.crop_image(cur_image_v, np.reshape(scene_boxes[bidx],(1,4)), crop_img_size)
        cropped_image_v = cropped_image_v - 128.

        cropped_image_i, cur_image_var_i = img_crop_model.crop_image(cur_image_i, np.reshape(scene_boxes[bidx],(1,4)), crop_img_size)
        cropped_image_i = cropped_image_i - 128.


        feat_map_v, feat_map_i, fused_feats = model(cropped_image_v, cropped_image_i, out_layer='conv3')
    ...
    for iidx in range(replicateNum):
        ...
        cur_extra_cropped_image_v, _ = img_crop_model.crop_image(cur_image_v, np.reshape(extra_scene_box,(1,4)), extra_crop_img_size)
        cur_extra_cropped_image_v = cur_extra_cropped_image_v.detach() 

        cur_extra_cropped_image_i, _ = img_crop_model.crop_image(cur_image_i, np.reshape(extra_scene_box,(1,4)), extra_crop_img_size)
        ...
        if iidx==0:
            ...
            extra_cropped_image_v = torch.cat((extra_cropped_image_v, cur_extra_cropped_image_v),dim=0)
            extra_cropped_image_i = torch.cat((extra_cropped_image_i, cur_extra_cropped_image_i),dim=0)
    ...
    for iidxxx in range(replicateNum):
        temp_extra_cropped_image_v = torch.unsqueeze(extra_cropped_image_v[iidxxx], dim=0) 
        temp_extra_cropped_image_i = torch.unsqueeze(extra_cropped_image_i[iidxxx], dim=0) 
        temp_extra_feat_maps_v, temp_extra_feat_maps_i, temp_extra_feat_maps = model(temp_extra_cropped_image_v, temp_extra_cropped_image_i, out_layer='conv3') 
        ...
        if iidxxx == 0: 
            extra_feat_maps = torch.zeros(replicateNum, temp_extra_feat_maps.shape[0], temp_extra_feat_maps.shape[1], temp_extra_feat_maps.shape[2])
    ...
    for i in range(1, len(img_list_v)):
        ...
        if failure_count >= 8 and attentionFlag: 
            
            attentionMap = Image.open(attention_path+attentionImage_name).convert('RGB')
            attentionMap = np.asarray(attentionMap)
            # pdb.set_trace() 

            dynamic_atttentonMAP = cv2.resize(attentionMap, (cur_image_v.shape[1], cur_image_v.shape[0]), interpolation=cv2.INTER_LINEAR)
            ret, static_atttentonMAP = cv2.threshold(dynamic_atttentonMAP, 100, 255, cv2.THRESH_BINARY) 
            ...
            for iii in range(attNum):
                ...
                if math.fabs(center_position[0] - centerPos_prev_x) < 30 and math.fabs(center_position[1] - centerPos_prev_y) < 30: 
                    ...
                    switch_samples2 = gen_samples(SampleGenerator('gaussian', (ishape[1], ishape[0]), trans_f, opts['scale_f'], valid=True), new_bbox2, 256)
        ...
        crop_img_size = (padded_scene_box[2:4] * ((opts['img_size'], opts['img_size']) / target_bbox[2:4])).astype('int64')
        cropped_image_v, cur_image_var_v = img_crop_model.crop_image(cur_image_v, np.reshape(padded_scene_box,(1,4)), crop_img_size)
        cropped_image_v = cropped_image_v - 128.
        cropped_image_i, cur_image_var_i = img_crop_model.crop_image(cur_image_i, np.reshape(padded_scene_box,(1,4)), crop_img_size)
        cropped_image_i = cropped_image_i - 128.

        model.eval()
        feat_map_v, feat_map_i, feat_map = model(cropped_image_v, cropped_image_i, out_layer='conv3')
